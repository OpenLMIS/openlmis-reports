...
sudo mkdir -p /var/www/html/erd-report
sudo chown -R $USER:$USER /var/www/html/erd-report
...
&& mkdir output \
&& chmod 777 output \
&& (docker run --rm --network ${COMPOSE_PROJECT_NAME//.}_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s report -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
&& sleep 30 \
&& /usr/local/bin/docker-compose down --volumes \
&& sudo chown -R $USER:$USER output \
&& mv output/* /var/www/html/erd-report \
&& rm erd-report.zip -f \
&& pushd /var/www/html/erd-report \
&& zip -r $WORKSPACE/erd/erd-report.zip . \
&& popd \
&& rmdir output \
&& rm .env \
