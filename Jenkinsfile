...
pipeline {
    ...
    environment {
        COMPOSE_PROJECT_NAME = "report${BRANCH_NAME}"
    }
    parameters {
    ...
    stages {
        ...
        stage('Deploy to test') {
            ...
            steps {
                build job: 'OpenLMIS-report-deploy-to-test', wait: false
            }
            post {
                failure {
                    script {
            ...
            parallel {
                ...
                stage('Contract tests') {
                    steps {
                        ...
                        parameters: [
                            string(name: 'serviceName', value: 'report'),
                            text(name: 'customEnv', value: "OL_REPORT_VERSION=${STAGING_VERSION}")
                        ]
                    }
                    post {
                        failure {
        ...
        stage('ERD generation') {
            ...
            steps {
                dir('erd') {
                    ...
                    archiveArtifacts artifacts: 'erd-report.zip'
                }
            }
            post {
                failure {
